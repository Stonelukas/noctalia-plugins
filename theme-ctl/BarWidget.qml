...
Item {
  ...
  readonly property string pluginTitle: pluginApi?.tr("title") || "Theme-ctl"
  readonly property string unavailableLabel: pluginApi?.tr("status.not-available") || "Theme-ctl cache not found"
  ...
  readonly property string tooltipText: {
    if (!isActive)
      return pluginApi?.tr("tooltips.inactive") || "Theme-ctl (inactive)\nClick to open settings";
    if (!isAvailable)
  ...
  function openPluginSettings() {
    ...
    function instantiateDialog(component) {
      ...
      if (!dialog) {
        Logger.e("ThemeCtlWidget", "Failed to instantiate plugin settings dialog:", component.errorString());
        return;
      }
    ...
    if (settingsPopupComponent.status === Component.Ready) {
      ...
      var handler = function settingsComponentStatusChanged() {
        if (settingsPopupComponent.status === Component.Ready) {
          ...
          Logger.e("ThemeCtlWidget", "Failed to load plugin settings dialog:", settingsPopupComponent.errorString());
          settingsPopupComponent.statusChanged.disconnect(handler);
  ...
  function selectRandomTheme() {
    ...
    if (themes.length === 0) {
      Logger.w("ThemeCtlWidget", "No themes available");
      return;
    }
    ...
    if (otherThemes.length === 0) {
      Logger.w("ThemeCtlWidget", "No other themes available to switch to");
      return;
    }
    ...
    Logger.d("ThemeCtlWidget", "Random theme:", randomName);
